...
dataPath = "./pre-training-dataset.xlsx"  # 엑셀 파일 경로

exel_row_name = ["text"]
hf_row_name = ["text"]


# 엑셀 파일을 DataFrame으로 변환하는 함수
def excel_to_dataframe(excel_path):
    df = pd.read_excel(excel_path)
    print(f"Loaded {excel_path} into DataFrame")
    return df


# DataFrame을 JSON 데이터로 변환하는 함수
def dataframe_to_json(df):
    json_data = []
    for index, row in df.iterrows():
        data = {exel_row_name[0]: row[exel_row_name[0]]}
...
dataset_name = "BaekSeungJu/Ophthalmology-PubMed-Corpus"  # Hugging Face에 업로드할 데이터셋 이름을 입력
